...
def collect_detections(predict_fn, samples, score_floor=0.02, nms_thr=0.4):
...
def evaluate(predict_fn, samples, score_floor=0.02, nms_thr=0.4, iou_thr=0.5):
    """AP@iou_thr per GT face-size bucket. Returns (AP per bucket, GT count
    per bucket, ms/image)."""
    records, ms = collect_detections(predict_fn, samples, score_floor, nms_thr)
    result, n_gt = {}, {}
...
def evaluate_full(predict_fn, samples, score_floor=0.02, nms_thr=0.4, iou_thrs=None):
...
class TorchValRunner:
    """Wraps a live KFaceDetector for evaluate() without an ONNX round-trip."""

    def __init__(self, model, anchors, size, device, conf_thresh=0.02, nms_thresh=0.4):
        self.model = model
    ...
    def __call__(self, img_bgr):
        ...
        x = torch.from_numpy(inp.astype(np.float32)).to(self.device)

        boxes, _kps, scores = self.model.predict(
            x, anchors=self.anchors, conf_thresh=self.conf_thresh, iou_thresh=self.nms_thresh
        )[0]
        return (boxes / scale).cpu().numpy(), scores.cpu().numpy()
